 ...
 def run(self,prompt,max_steps=20):
  ...
  for i in range(max_steps):
   ...
   if self.approval and not self.approval.request(self.session_id,name,args):
    ...
    if hasattr(self.executor,"job") and self.executor.job: self.executor.job.manager.set_waiting(self.executor.job)
    if not self.approval.wait_for(self.session_id,name,args): return "APPROVAL_DENIED_OR_TIMEOUT: "+name
    if hasattr(self.executor,"job") and self.executor.job: self.executor.job.manager.set_running(self.executor.job)
